...
class Environment:
    ...
    def reset(self):
        ...
        当前无人机个数为 1
        """
        obs = np.empty(shape=(1, 5))
        obs[0][0] = self.uav_location[0]
        obs[0][1] = self.uav_location[1]
        obs[0][2] = self.uav_location[2]
        obs[0][3] = 5  # 传输速率1
        obs[0][4] = 5  # 传输速率2
        # min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0, 1))
        # obs_normalization = min_max_scaler.fit_transform(obs)
        obs_normalization = obs
        obs_normalization[0][0] = obs[0][0] / self.X  # (X-X_min)/(X_max-X_min)
        obs_normalization[0][1] = obs[0][1] / self.Y  # 500
        obs_normalization[0][2] = obs[0][2] / self.Z  # 200
        obs_normalization[0][3] = obs[0][3] / 100
        obs_normalization[0][4] = obs[0][4] / 100
    ...
    def step(self, cur_point, obs, action):
        ...
        next_obs = np.empty(shape=(1, 5))
        next_obs[0][0] = self.dcp_location[cur_point - 1][0]
        next_obs[0][1] = self.dcp_location[cur_point - 1][1]
        next_obs[0][2] = self.dcp_location[cur_point - 1][2]
        next_obs[0][3] = self.get_comm_rate(next_obs[0][:3], action[0][1], self.vsp_location[0])
        next_obs[0][4] = self.get_comm_rate(next_obs[0][:3], action[0][1], self.vsp_location[1])

        # next_obs[0][3] = np.random.randint(1, 5)  # 功率
        Q = self.data_volume[cur_point - 1]
        comm_delay_first = self.get_comm_delay(Q, action[0][0], next_obs[0][:3], action[0][1], self.vsp_location[0])
        comm_delay_second = self.get_comm_delay(Q, 1 - action[0][0], next_obs[0][:3], self.max_power_UAV - action[0][1],
                                                self.vsp_location[1])
        print(f"uav_location={next_obs[0][:3]}, comm_rate1={next_obs[0][3]}, comm_rate2={next_obs[0][4]}")
        print(f'task ratio={action[0][0]}, uav_power={action[0][1]}, comm_delay_first={comm_delay_first}, comm_delay_second={comm_delay_second}')
        reward = -max(comm_delay_first, comm_delay_second)

        if cur_point == self.dcp_num:
            done = True

        next_obs_normalization = next_obs
        next_obs_normalization[0][0] = next_obs[0][0] / self.X  # (X-X_min)/(X_max-X_min)
        next_obs_normalization[0][1] = next_obs[0][1] / self.Y  # 500
        next_obs_normalization[0][2] = next_obs[0][2] / self.Z  # 200
        next_obs_normalization[0][3] = next_obs[0][3] / 100
        next_obs_normalization[0][4] = next_obs[0][4] / 100
        # min_max_scaler = preprocessing.MinMaxScaler(feature_range=(0, 1))
